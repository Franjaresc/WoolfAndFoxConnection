...
const validateOrderData = ({ Id, Date, Type, Company, Price, Observation }) => {
    ...
    if (!Date || !Type || !Company || typeof Price !== 'number') {
        throw new Error("Invalid order data. Ensure all fields are filled correctly.");
    }
};
...
export const insertOrder = async (orderData) => {
    try {
        ...
        const response = await sql`
            INSERT INTO public."Orders" ("Id", "Date", "Type", "Company", "Price", "Observation")
            VALUES (${orderData.Id}, ${orderData.Date}, ${orderData.Type}, ${orderData.Company}, ${orderData.Price}, ${orderData.Observation}) 
            RETURNING "Id";`;

        return response.rows[0].Id; // Devuelve el ID de la nueva orden
